...
def build_legacy_control_export_notebook(
    commit_sha: str,
    *,
    ...
    cells = [
        nbformat.v4.new_markdown_cell(
            ...
            "This bounded A100 comparison reproduces the passing legacy-control "
            "5K diagnostic once. It stages the verified lineage once, trains one "
            "deterministic arm, preserves the trainer's raw PLY byte-for-byte, "
            "runs the unchanged production PLY polish, and publishes both PLYs "
            "with the polish report. A rejected polish candidate is still kept "
            "for inspection. It never starts a full 120K run and never overwrites "
            "the learned, legacy, ablation, or floor-diagnostic result.",
            metadata=_tag("title"),
        ),
    ...
    notebook = nbformat.v4.new_notebook(
        ...
        metadata={
            ...
            "generator": {
                "id": "4dgs-studio.legacy-control-export-notebook",
                "version": 1,
...
def write_legacy_control_export_notebook(path: Path, commit_sha: str) -> Path:
    notebook = build_legacy_control_export_notebook(commit_sha=commit_sha)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate the legacy-control dual PLY notebook"
    )
    parser.add_argument("--commit", required=True)
    parser.add_argument("--output", required=True, type=Path)
    args = parser.parse_args(argv)
    write_legacy_control_export_notebook(args.output, args.commit)
    return 0
